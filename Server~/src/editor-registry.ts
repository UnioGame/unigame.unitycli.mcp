...
export interface EditorRegistrySnapshot {
  ...
  stale_editors: Array<EditorMetadata & { stale_reason: string }>;
  corrupt_entries: Array<{ path: string; error: string }>;
}
...
const metadataKeys = new Set<keyof EditorMetadata>([
...
const stringKeys: Array<keyof EditorMetadata> = [
...
export function validateEditorMetadata(value: unknown): EditorMetadata {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("metadata must be an object");
  const object = value as Record<string, unknown>;
  for (const key of Object.keys(object))
    if (!metadataKeys.has(key as keyof EditorMetadata))
  ...
  return object as unknown as EditorMetadata;
}

const execFileAsync = promisify(execFile);

export async function processMatchesStart(
  pid: number,
  startedAtUtc: string,
): Promise<boolean> {
  try {
...
async function defaultProcessMatches(metadata: EditorMetadata): Promise<boolean> {
  return processMatchesStart(metadata.editor_pid, metadata.editor_started_at_utc);
}

async function descriptorIsReadable(metadata: EditorMetadata): Promise<boolean> {
  try {
...
export async function discoverEditors(
  ...
  snapshot.active_editors.sort((a, b) =>
    a.project_id.localeCompare(b.project_id) ||
    a.editor_instance_id.localeCompare(b.editor_instance_id));
  return snapshot;
}

function matchesPath(editor: EditorMetadata, path: string): boolean {
...
export async function resolveEditor(
  ...
  const selector =
    selectors.editor_instance_id ? ["editor_instance_id", selectors.editor_instance_id] as const :
    selectors.project_id ? ["project_id", selectors.project_id] as const :
    selectors.project_path ? ["project_path", selectors.project_path] as const :
    selectors.projectPath ? ["project_path", selectors.projectPath] as const :
    fallbackPath ? ["project_path", fallbackPath] as const :
    null;
  if (selector) {
    const [key, value] = selector;
    const predicate = key === "project_path"
      ? (editor: EditorMetadata) => matchesPath(editor, value)
      : (editor: EditorMetadata) => editor[key] === value;
