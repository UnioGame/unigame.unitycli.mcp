...
export function parseOfficialClientList(value: unknown): Map<AgentId, boolean> {
  const result = new Map<AgentId, boolean>();
  const root = value as Record<string, unknown> | unknown[] | null;
...
export function discoverAgents(
  ...
  official = new Map<AgentId, boolean>(),
): AgentRegistration[] {
  const appData =
...
export function registrationValue(
  ...
  broker: { token_file: string; port: number },
...
function adapter(
  ...
  display_name: string,
  fallback: string,
  format: AgentRegistration["format"],
  key: AgentRegistration["key"],
