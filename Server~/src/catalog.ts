...
export async function loadCatalogs(): Promise<ToolCatalog[]> {
  return Promise.all(
    catalogFiles.map(async (file) =>
      JSON.parse(await readFile(join(catalogDirectory, file), "utf8")) as ToolCatalog,
    ),
  );
}
...
function parameterSchema(parameter: CatalogParameter): Record<string, unknown> {
...
export function inputSchema(tool: CatalogTool): Record<string, unknown> {
...
export async function resolveUnityCli(): Promise<string | null> {
