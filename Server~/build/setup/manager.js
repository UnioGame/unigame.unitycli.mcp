...
import { discoverAgents, parseOfficialClientList, registrationValue, supportedAgents, } from "./agents.js";
import { fingerprint, managedTomlBlock, managedTomlFingerprint, parseJsonc, patchManagedToml, patchServerJsonc, } from "./config.js";
...
async function inspect(context) {
    ...
    const [version, status, officialClients, registry, broker] = await Promise.all([
        ...
        cli_path
            ? safeRun(cli_path, ["status", "--project", context.project_path, "--format", "json", "--no-banner"], 10_000)
            : Promise.resolve(null),
    ...
    const agent_statuses = await Promise.all(agents.map((agent) => inspectAgent(context, agent)));
    const skill_statuses = await Promise.all(skillIds.map((id) => inspectSkill(context, id)));
    const pipeline_version = await installedPipelineVersion(context.project_path);
    const current_editor = registry.active_editors.find((editor) => resolve(editor.project_path) === context.project_path) ?? null;
    ...
    return {
        ...
        response_data: {
            unity_cli: {
                ...
                path: cli_path,
                version: version?.exitCode === 0 ? redact(version.stdout.trim(), 1_000) : null,
            ...
            official_mcp: {
                ...
                state: cli_path && pipeline_version && editor_ready
                    ? "ready"
                    : "not_ready",
...
async function buildPlan(context, request, snapshot) {
    ...
    const enable_agents = snapshot.agents.filter((entry) => enabledAgentIds.includes(entry.agent_id));
    const disable_agents = snapshot.agents.filter((entry) => disabledAgentIds.includes(entry.agent_id));
    const changes = [];
    const warnings = [];
    const paths = new Set();
    if (request.operation !== "remove") {
        ...
        for (const agent of enable_agents) {
            const state = snapshot.agent_statuses.find((entry) => entry.agent_id === agent.agent_id);
            changes.push({
...
async function applyPlan(context, request, snapshot, plan) {
    ...
    for (const agent of plan.enable_agents) {
        const status = snapshot.agent_statuses.find((entry) => entry.agent_id === agent.agent_id);
        if (status.registration_state === "conflict" && !request.force)
            throw new Error(`CONFLICT:${agent.agent_id}`);
        const value = registrationValue(context, snapshot.cli_path ?? "", request.transport ?? "stdio", {
...
async function discoverOfficialClients(cliPath) {
    if (!cliPath)
        return new Map();
    const result = await safeRun(cliPath, ["mcp", "configure", "--list", "--format", "json", "--no-banner"], 10_000);
...
async function hasLegacyGlobal(agent) {
    if (!existsSync(agent.config_path))
        return false;
...
async function isManaged(context, agent) {
    ...
    if (marker?.managed_by !== "com.unigame.unitycli.mcp" ||
        marker.registration_name !== context.registration_name ||
        !marker.fingerprint)
        return false;
    if (agent.format === "toml")
        return (managedTomlFingerprint(await readText(agent.config_path), context.registration_name) === marker.fingerprint);
...
async function writeRegistration(context, agent, value) {
    ...
    if (agent.format === "toml") {
        ...
        const block = value
            ...
            : "";
        await atomicWrite(agent.config_path, patchManagedToml(text, context.registration_name, block));
    }
    else {
        await atomicWrite(agent.config_path, patchServerJsonc(text, agent.key, context.registration_name, value));
    }
    if (!value) {
        await rm(markerPath(context, agent.agent_id), { force: true });
        return;
    }
    await atomicWrite(markerPath(context, agent.agent_id), JSON.stringify({
...
function registrationFingerprint(agent, value) {
    if (agent.format !== "toml" || "url" in value)
        return fingerprint(value);
    return fingerprint({ ...value, env: value.env ?? {} });
}
const skillIds = ["operate-unity-cli", "operate-unity-mcp"];
async function inspectSkill(context, id) {
    ...
    for (const target of targets) {
        ...
        const targetManaged = manifest?.managed_by === "com.unigame.unitycli.mcp" &&
...
async function installSkill(context, id) {
    ...
    for (const target of skillTargets(context, id)) {
        ...
        if (id === "operate-unity-mcp") {
            const capabilityMap = join(context.package_root, "Documentation~", "unity-cli-capabilities.md");
            await mkdir(join(temporary, "references"), { recursive: true });
            await copyFile(capabilityMap, join(temporary, "references", "unity-cli-capabilities.md"));
        }
        await writeFile(join(temporary, ".unigame-managed.json"), JSON.stringify({
            managed_by: "com.unigame.unitycli.mcp",
            skill_id: id,
            version: toolkitVersion,
            source_hash,
...
async function skillSourceHash(context, id) {
    const extras = id === "operate-unity-mcp"
        ? [{
                ...
                source_path: join(context.package_root, "Documentation~", "unity-cli-capabilities.md"),
            }]
        : [];
...
function selectedAgentIds(request, agents) {
    ...
    if (request.target_kind === "agent" && request.target_id)
        return supportedAgents.includes(request.target_id)
            ? [request.target_id]
            : [];
    if (request.agent_ids)
        return request.agent_ids;
...
function selectedSkillIds(request) {
    ...
    if (request.target_kind === "skill" && request.target_id)
        return skillIds.includes(request.target_id)
            ? [request.target_id]
            : [];
...
async function installBundle(context) {
    ...
    await cp(join(context.package_root, "Server~", "catalogs"), join(temporary, "catalogs"), { recursive: true });
    await cp(join(context.package_root, "Server~", "schemas"), join(temporary, "schemas"), { recursive: true });
    await mkdir(dirname(target), { recursive: true });
    await rm(target, { recursive: true, force: true });
    await rename(temporary, target);
    await atomicWrite(join(context.install_root, "current.json"), JSON.stringify({
        version: toolkitVersion,
...
async function serve(context, request, response) {
    ...
    const ownerStartedAtUtc = request.owner_started_at_utc ??
        (ownerPid === process.pid ? processStartedAtUtc() : null);
    const leaseId = (request.editor_instance_id ??
        (ownerPid === process.pid ? randomUUID() : "")).replace(/[^a-zA-Z0-9._-]/g, "_");
    if (!leaseId)
    ...
    try {
        ...
        const child = spawn(process.execPath, [
            ...
            env: {
                ...process.env,
                UNIGAME_UNITYCLI_ROOT: join(context.install_root, "versions", toolkitVersion),
...
async function brokerStatus(context) {
    const state = (await readJson(statePath(context)));
    const leases = await liveBrokerLeases(join(context.install_root, "broker-leases"), { cleanupStale: false });
    return {
        opt_in: true,
...
function installedServer(context) {
    return join(context.install_root, "versions", toolkitVersion, "dist", "index.js");
}
function statePath(context) {
    return join(context.install_root, "http-state.json");
}
function markerPath(context, id) {
    return join(context.install_root, "registrations", `${context.registration_name}.${id}.json`);
}
    ...
    async function visit(path) {
        ...
        if (details.isDirectory()) {
            ...
            for (const entry of entries) {
                if (entry.name === ".unigame-managed.json" ||
