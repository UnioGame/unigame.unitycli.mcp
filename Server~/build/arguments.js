...
function appendParameter(result, source, parameter, value) {
    if (value === undefined || value === null || metaNames.has(parameter.name))
        return;
    if (parameter.positional) {
        if (Array.isArray(value))
            result.push(...value.map(serialize));
        else
    ...
    if (Array.isArray(value) &&
        ...
        for (const entry of value)
            result.push(option, serialize(entry));
        return;
    }
    result.push(option, serialize(value));
}
export function buildArguments(tool, input) {
    const args = [];
    let target = null;
    if (tool.source === "cli") {
