...
| Connected tools | `status`, `list`, `command` | Discover running Editors, return actual tool schemas, and invoke one command. Target an Editor with `--project-path`; target a Player with `--runtime` or `--runtime-path`. `command` timeout defaults to 30 seconds. **Pipeline transport, verified.** |
